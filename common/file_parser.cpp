...
IniFileParser::IniFileParser(const char* sIniFile)
{
//	DBG_PRINT("Run %s(const char* sIniFile) ...\n", __FUNCTION__);
	this->Load(sIniFile);
}
...
bool IniFileParser::Load(const char* sIniFile)
{
//	DBG_PRINT("Run %s() ...\n", __FUNCTION__);
	bzero(m_sFilePath, sizeof(m_sFilePath));
	strncpy(m_sFilePath, sIniFile, FILE_UTIL_FILE_PATH_MAX);

	if (FileUtil::FileExist(m_sFilePath) == false) {
		DBG_PRINT("File (%s) doesnt exist!\n", m_sFilePath);
		return true;
	}
